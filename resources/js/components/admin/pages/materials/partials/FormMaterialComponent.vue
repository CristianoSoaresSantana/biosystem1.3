              ...
              <div class="form-row">
                <div class="form-group col-md-6">
                  <div :class="['col-auto', {'has-error': errors.forma_farmaceutica_id}]">
                    ...
                    <select
                      class="form-control mb-2 mr-sm-2"
                      v-model="material.forma_farmaceutica_id"
                    >
                      <option value>Forma Farmacêutica</option>
                      <option
                        v-for="forma in forma_farmaceuticas"
                        :key="forma.id"
                        :value="forma.id"
                      >{{forma.forma_farmaceutica}}</option>
                    </select>
                  </div>
                </div>
                <div class="form-group col-md-6">
                  <div :class="['col-auto', {'has-error': errors.tipo_material_id}]">
                    ...
                    <select class="form-control mb-2 mr-sm-2" v-model="material.tipo_material_id">
                      ...
                      <option
                        v-for="tipo in tipo_materials"
                        :key="tipo.id"
                        :value="tipo.id"
                      >{{tipo.tipo_material}}</option>
                    </select>
                  </div>
                </div>
              </div>
              ...
              <div class="form-row">
                <div class="form-group col-md-6">
                  <div :class="['col-auto', {'has-error': errors.valor_revenda}]">
                    ...
                    <input
                      type="text"
                      v-model="material.valor_revenda"
                      class="form-control mb-2 mr-sm-2"
...
export default {
  ...
  methods: {
    onSubmit() {
      ...
      this.$store
        .dispatch(createOrUpdate, this.material)
        .then(() => {
